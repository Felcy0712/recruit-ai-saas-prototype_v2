...
export function SplitScreenBeforeAfter() {
  ...
  const [candidateQualityInView, setCandidateQualityInView] = useState(false)
  const [scrollY, setScrollY] = useState(0)
  const sectionRef = useRef<HTMLDivElement>(null)
  const whatsappSectionRef = useRef<HTMLDivElement>(null)
  const voiceSectionRef = useRef<HTMLDivElement>(null)
  const serviceSectionRef = useRef<HTMLDivElement>(null)
  const CandidateQualitySectionRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          } else if (entry.target === CandidateQualitySectionRef.current) {
            setCandidateQualityInView(true)
          }
        }
      })
    }, observerOptions)
    ...
    if (CandidateQualitySectionRef.current) observer.observe(CandidateQualitySectionRef.current)
  ...
  const candidateQualityParallax = getParallaxOffset(CandidateQualitySectionRef)
  return (
          ...
          <div ref={whatsappSectionRef} className="mt-16 lg:mt-24 pt-16 lg:pt-24 border-t border-slate-200">
            ...
            <div ref={serviceSectionRef} className="mt-12 lg:mt-16 pt-12 lg:pt-16 border-t border-slate-200">
              <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                ...
                <div
                  ref={CandidateQualitySectionRef}
                  className={`order-2 lg:order-1 transition-all duration-1000 ease-out ${
                    candidateQualityInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
                >
