...
const getGradientDirection = (position: string): string =>
  ({ top: "to top", bottom: "to bottom", left: "to left", right: "to right" } as Record<string, string>)[position] || "to bottom"
...
function GradualBlur(props: GradualBlurProps) {
  ...
  // ✅ Fix: was missing `const config = useMemo(() => {`
  ...
  const isScrollAnimated = config.animated === "scroll"
  const isVisible = useIntersectionObserver(containerRef, isScrollAnimated)
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      divs.push(
        <div key={i} style={{
          ...
          transition: config.animated && !isScrollAnimated
    ...
    return divs
  }, [config, isHovered, isScrollAnimated])
  ...
  const { hoverIntensity, onAnimationComplete, duration } = config

  useEffect(() => {
    if (isVisible && isScrollAnimated && onAnimationComplete) {
      const ms = Number.parseFloat(duration) * 1000
      const t = setTimeout(() => onAnimationComplete(), ms)
      return () => clearTimeout(t)
    }
  }, [isVisible, isScrollAnimated, onAnimationComplete, duration])
...
// ✅ Fix: cast to any to allow attaching static properties
